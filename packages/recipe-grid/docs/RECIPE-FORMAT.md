...
Absent frontmatter defaults to `{ scalingType: 'fixed', base: 1, unitSystem: 'us' }`, so a recipe with no metadata is still valid. `servings` can be used downstream to update values as a multiplier of `base` client-side only; `fixed` does not scale.

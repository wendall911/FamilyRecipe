...
FamilyRecipe is a monorepo for a greenfield tabular flexbox layout for recipes. This is a novel approach to recipes that uses the Cooking for Engineers approach of a single card for recipes, not a list of recipes followed by instructions. It is a novel project; make no assumptions about how recipes work or what they mean.

A DAG is the only representation that can accurately describe the flexbox layout. A Markdown file contains a human-readable DAG recipe with edges that extend to other recipes. The parser produces an AST that represents the DAG. The compiler produces a DAG. A DOM chunk is extracted from the compiled DAG that also fully represents a DAG. There is no tree, only a DAG.

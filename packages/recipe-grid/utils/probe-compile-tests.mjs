// Compile the -svelte test fixture through the real parser + compiler and print
// the DAG, so it can be checked against the AST and the walked structure.
...
for (const name of ['turkish-pizza']) {
  const md = readFileSync(`${PKG}/../recipe-grid-svelte/tests/fixtures/${name}.md`, 'utf8');
